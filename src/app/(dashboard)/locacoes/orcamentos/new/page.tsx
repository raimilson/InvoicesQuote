...
export default function NewOrcamentoPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setProducts(Array.isArray(p) ? p.filter((prod: Product) => prod.active) : []);
    });
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/locacoes/orcamentos", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      toast.error("Falha ao criar orcamento");
    } finally {
      setLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Novo Orcamento</h1>
        </div>
